...
class CStyleStructure(object, metaclass=CMeta):
    """
        ...
        for d, obj in self.__dict__.items():
            try:
                if isinstance(obj, tuple):
                    if issubclass(obj[0], CStyleStructure):
                        _type, count = obj
            ...
            try:
                ...
                elif issubclass(obj, CStyleStructure):
    ...
    def _link_cstructs(self, obj):
        ...
        for fname, subobj in obj.__filtermap__.items():
            ...
            elif isinstance(subobj, CStyleStructure):
        ...
        if struct:
            ...
            for fn, val in fields:
                if fn == name:
                    ...
                    return
        super(CStyleStructure, self).__setattr__(name, value)
    ...
    def __getattribute__(self, name):
        ...
        """
        try:
            struct = super(CStyleStructure, self).__getattribute__('__struct__')
            if struct:
                fields = struct._fields_
                for fn, val in fields:
                    if fn == name:
                        val = struct.__getattribute__(name)
                        tests = (CStyleStructure.FilteredStruct, CStyleUnion.FilteredStruct, ct.Array)
                        if any([isinstance(val, t) for t in tests]):
                            fm = super(CStyleStructure, self).__getattribute__('__filtermap__')
            ...
            pass
        return super(CStyleStructure, self).__getattribute__(name)


class CStyleUnion(CStyleStructure, metaclass=CMeta):
